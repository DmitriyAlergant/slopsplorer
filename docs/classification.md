...
This document states the rules that produce them, and the reasons for the rules that are not obvious.
...
2. `isSourceFile()` accepts it.
   ...
   Where it is committed, or where `--all-files` is used, it is scanned and marked generated instead, so its weight stays visible behind one switch.
   `--exclude <name>` adds more folder names.
3. It is not larger than `--max-file-bytes`, which is 2 MiB by default.

`SOURCE_EXTENSIONS` is an allow list, so a binary or a media file never enters a snapshot and never has to be detected as one.
The cost of the allow list is that a file type nobody added is invisible.
Several formats have comment rules in `lines.ts` that a scan never reaches for this reason, among them Terraform, INI, SVG, `Dockerfile`, and every script with no extension.
Widening the scan is a separate decision about what belongs on the map.
...
`generated` is a separate boolean, not a flavor, because a generated file is still code or still data.
...
2. A `.json`, `.yaml`, or `.yml` file whose name is a language code, with an optional region, such as `de-DE.json` or `pt_BR.json`: `i18n`.
   The language codes are an explicit list. Accepting any two or three letter name would file `api.json`, `db.yaml`, and `dev.yaml` as translations.
...
### Why the test folder ranks below the format

The two halves of test detection are not equally strong.

A filename such as `service.test.ts` states what the file is, so it ranks above the extension.
A folder named `tests` states only where the file is, and a test folder holds fixtures, sample documents, and recorded input beside its code.
So the folder rule applies to code extensions only.
A JSON fixture in `tests/fixtures/` is `data`, a recorded clipboard payload in `tests/paste/msword_clipboard.html` is `other`, and `tests/utils/websocket_client.py` is still `test`.

The cost is that hiding the tests no longer hides the weight of the fixtures beside them.
That weight was measured on one large repository: the files that this rule moves out of `test` are about 3 percent of the weight of the test folders, so the totals hardly change while the file table now names each file for what it is.
...
A translation catalogue written as a TypeScript module has a code extension and no code in it.
Its weight grows with its content, not with its logic, so `refineKindByContent()` re-files it.
...
Each condition removes a real false positive that was measured over 1,855 parsed code files:

- Shell scripts quote nearly everything they handle, including paths, messages, and heredocs of SQL. The family sits between 70 and 97 percent literal content while still being scripts.
- Small files reach any ratio by accident.
- An icon component whose SVG path attributes reach 88 percent is still a component.
- One large template literal of injected script, or one embedded SVG, reaches 99 percent literal content in a file that is clearly code. The dominance test is what separates a table of many strings from one large embedded blob.

On that repository the complete rule re-files exactly one file out of 2,045.
...
A docstring counts as comment and not as literal, because it is already commentary.

A file with no grammar has no literal measurement, so this rule never applies to it.

## Generated output

`isGenerated()` decides from the path alone, so no file has to be read to be discounted.
...
Fish is deliberately not sent there. It uses `#` comments, but its syntax is not Bourne shell, so it takes the marker table instead.
...
These counts are output that a reader has to be able to check, so the table is explicit rather than a name pattern.

`branches` counts decision points.
A clause that the grammar puts inside its parent, such as `else_clause` or `catch_clause`, is left out to avoid counting one decision twice, while a clause that is a real extra decision, such as `elif_clause`, is counted.
...
A Python docstring counts as comment, because Python has no block-comment syntax and a docstring carries the weight that `/* */` carries elsewhere.
This is a deliberate difference from `cloc`, which counts a docstring as code.
...
It also tracks string literals, because one `/*` inside a quoted value would otherwise open a comment that runs to the end of the file.
...
That is the purpose of the fallback: a file with content must never report zero of everything.
Markdown and JSON are left out of the table on purpose. A Markdown paragraph is content and not commentary, and JSON has no comment syntax.
